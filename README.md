...
[![Starlight](https://img.shields.io/badge/Starlight-0.42-8B5CF6)](https://starlight.astro.build)
...
- **Multilingue** — Disponible en Français, Anglais, Espagnol et Allemand
...
- **Performant** — Site statique ultra-rapide avec Astro & Starlight
...
├── Métiers      # Liste des métiers disponibles, avec tarifs en direct
...
```
Wiki/
├── public/                        # Assets statiques (favicon, images, scripts)
├── src/
│   ├── assets/                    # Logo et images utilisées par la config Starlight
│   ├── content/
│   │   ├── docs/                  # Tout le contenu du wiki (fr à la racine, en/, es/, de/)
│   │   │   └── jobs/*.mdx         # Pages métiers (texte + données dynamiques)
│   │   └── content.config.ts      # Collections `docs` (Starlight) et `jobsData`
│   ├── components/
│   │   ├── JobCard.astro          # Carte d'aperçu d'un métier (grille /jobs/)
│   │   ├── JobIcon.astro          # Icônes Lucide associées à chaque métier
│   │   ├── JobPricingTable.astro  # Rendu générique des données de métiers
│   │   └── Carousel.astro         # Carrousel d'images réutilisable
...
| `bun run check` | Vérifie les types (`astro check`) |
...
**Créé avec passion par Arkyan**

[Discord](https://discord.gg/NdWQ3XfFEn) • [Site Web](https://capitalyx.fr)
